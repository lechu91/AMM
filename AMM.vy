...
def provideLiquidity(tokenA_addr: address, tokenB_addr: address, tokenA_quantity: uint256, tokenB_quantity: uint256):
	...
	self.tokenA.transferFrom(self.owner, self, tokenA_quantity)
	self.tokenAQty = tokenA_quantity
	self.tokenB = ERC20(tokenB_addr)
	self.tokenB.transferFrom(self.owner, self, tokenA_quantity)
...
def tradeTokens(sell_token: address, sell_quantity: uint256):
	...
	if sell_token == self.tokenA.address:
		...
		self.tokenB.transfer(msg.sender, self.tokenBQty - new_B)
	...
	if sell_token == self.tokenB.address:
		...
		self.tokenA.transfer(msg.sender, self.tokenAQty - new_A)
...
def ownerWithdraw():
    ...
    self.tokenA.transfer(self.owner, as_unitless_number(self.tokenAQty))
    self.tokenB.transfer(self.owner, as_unitless_number(self.tokenBQty))
